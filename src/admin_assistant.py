...
model = OpenAIChatModel(model_name='llama3.2', provider=OpenAIProvider(base_url='http://localhost:11434/v1'))
...
agent1 = Agent(
    model=model,
    system_prompt="You are a helpful Adminstrative Assistant.",  # System prompt to set agent behavior
)

# Example usage of basic agent"
response = agent1.run_sync("What time is the status of my first meeting tomorrow?")
...
agent2 = Agent(
    ...
    retries=10,
    system_prompt=(
        "You are an intelligent administrative assistant. "
        "Analyze queries carefully and provide structured responses."
    ),
)

response = agent2.run_sync("What time is my first meeting tomorrow?")
...
response = agent5.run_sync(
    user_prompt="What's the status of my last meeting?", deps=customer
)
...
def get_shipping_status(order_id: str) -> str:
    ...
    if shipping_status is None:
        raise ModelRetry(
            f"No meeting information found for last meeting {order_id}. "
            "Make to check the meetings subject for the correct meeting ID. "
...
# Example usage
response = agent5.run_sync(
    user_prompt="What's the meeting id and status of my last meeting?", deps=customer
)
